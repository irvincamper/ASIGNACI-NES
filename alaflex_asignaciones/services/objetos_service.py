...
class ObjetosService:
    ...
    def get_kpis(self) -> list[dict]:
        ...
        return [
            ...
            {"title": "Con devolución", "value": str(data["con_devolucion"]), "icon": "fa5s.undo", "color": "#F59E0B"},
            {"title": "Activos", "value": str(data["activos"]), "icon": "fa5s.check-circle", "color": "#16A05D"},
        ]

    def list_objetos(self, search: str = "", categoria: str = "Todas las categorías", estado: str = "Todos los estados") -> list[dict]:
        rows = self.repository.list_summary(search=search.strip(), categoria=categoria, estado=estado)
        return rows

    def list_categories(self) -> list[str]:
        return ["Todas las categorías", *self.repository.list_categories()]
